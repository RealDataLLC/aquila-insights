...
- Filter by industry
- Adjust time window (6, 12, 24 months)
- SF range line chart with YoY comparison
...
def aggregate_monthly(df, submarkets, industries, time_window_months):
    """
    ...
    time_window_months : int
        Lookback window (6, 12, or 24 months)

    Returns
    -------
    monthly_current : DataFrame
        Current period monthly aggregations with month_name
    monthly_prior : DataFrame
        Prior year monthly aggregations with month_name
    """
    # Exclude most recent month (incomplete data)
    max_date = df['date'].max()
    max_complete_month = max_date - pd.DateOffset(months=1)

    # Calculate cutoff dates
    cutoff_date = max_complete_month - pd.DateOffset(months=time_window_months)
    cutoff_prior = cutoff_date - pd.DateOffset(months=12)

    # Filter by submarkets
    df_filtered_current = df[df['applicable_markets'].isin(submarkets)].copy()

    # Filter by industries (if not "All")
    if industries != ['All'] and len(industries) > 0:
        df_filtered_current = df_filtered_current[df_filtered_current['industry'].isin(industries)]

    # Filter to current period (excluding most recent month)
    df_filtered_current = df_filtered_current[
        (df_filtered_current['date'] >= cutoff_date) &
        (df_filtered_current['date'] <= max_complete_month)
    ]

    # Aggregate current period
    monthly_current = df_filtered_current.groupby(pd.Grouper(key='date', freq='ME')).agg({
        'sf_low': 'sum',
        'sf_high': 'sum',
        'sf_avg': 'count'  # Count of requirements
    }).reset_index()
    monthly_current.rename(columns={'sf_avg': 'count'}, inplace=True)
    ...
    # Filter for prior year (same months, one year earlier)
    df_filtered_prior = df[df['applicable_markets'].isin(submarkets)].copy()

    if industries != ['All'] and len(industries) > 0:
        df_filtered_prior = df_filtered_prior[df_filtered_prior['industry'].isin(industries)]

    # Get same months from prior year
    df_filtered_prior = df_filtered_prior[
        (df_filtered_prior['date'] >= cutoff_prior) &
        (df_filtered_prior['date'] < cutoff_date)
    ]

    # Aggregate prior period
    monthly_prior = df_filtered_prior.groupby(pd.Grouper(key='date', freq='ME')).agg({
        'sf_low': 'sum',
        'sf_high': 'sum',
        'sf_avg': 'count'
    }).reset_index()
    monthly_prior.rename(columns={'sf_avg': 'count'}, inplace=True)

    # Add month name matching current year (e.g., Sep 2024 → "Sep 2025" for alignment)
        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    ...
                    # Time window
                    html.Label("Time Window:", style={'fontWeight': 'bold', 'fontFamily': AQUILA_FONT, 'marginTop': '20px'}),
                    dcc.RadioItems(
                        id='time-window',
                        options=[
        ...
        dbc.Col([
            dcc.Loading(
                ...
                children=[
                    ...
                    # Count Chart
                    dcc.Graph(id='count-chart', config={'displayModeBar': False})
                ]
            )
        ], width=9)
...
def update_charts(submarkets, industries, time_window):
    ...
    # Aggregate data
    monthly_current, monthly_prior = aggregate_monthly(df_global, submarkets, industries, time_window)
    ...
    sf_fig.update_layout(
        title=dict(
            text='SF Range (Low/High) with Year-over-Year Comparison',
...
def export_csv(n_clicks, submarkets, industries, time_window):
    """Export filtered data to CSV"""
    ...
    # Format date
    df_export['month'] = df_export['date'].dt.strftime('%Y-%m')

    # Select and rename columns
    df_export = df_export[[
        'month',
        'sf_low_current', 'sf_high_current', 'count_current',
        'sf_low_prior', 'sf_high_prior', 'count_prior'
    ]].rename(columns={
        'month': 'Month',
