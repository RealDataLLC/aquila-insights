...
│   ├── property-management/                 # Aquila Operating KPIs
...
│   │   ├── requirements_demand_by_tenant_size_cbd.html # Office demand by tenant size - CBD market
│   │   ├── requirements_demand_by_tenant_size_sw.html # Office demand by tenant size - Southwest market
│   │   ├── requirements_demand_by_tenant_size_nw.html # Office demand by tenant size - Northwest market
│   │   ├── requirements_demand_by_tenant_size_e.html # Office demand by tenant size - East market
│   │   ├── requirements_demand_by_tenant_size_c.html # Office demand by tenant size - Central market
...
### 6.5. googlesheets_combined.ipynb (Combined Historical Requirements Analysis)
...
**5. requirements_vs_absorption_office.html**
...
- **Colors:** Navy (Mega), Greenspace (50k-100k), Brass (25k-50k), Copper (10k-25k), Pennybacker (Sub 10k)
- **Purpose:** Shows how demand composition by tenant size has shifted over time
- Height: 650px, Width: 1000px

**Market-Specific Demand Charts:**
The same chart format broken out by submarket (CBD, SW, NW, E, C):
- **8. requirements_demand_by_tenant_size_cbd.html** - CBD market
- **9. requirements_demand_by_tenant_size_sw.html** - Southwest market
- **10. requirements_demand_by_tenant_size_nw.html** - Northwest market
- **11. requirements_demand_by_tenant_size_e.html** - East market
- **12. requirements_demand_by_tenant_size_c.html** - Central market

**Market Mapping Rules:**
- Rows can be counted toward multiple submarkets based on the MARKET field
- **Citywide/Flexible:** "Citywide", "Flexible", "Market Wide", "Austin MSA", "Austin Metro" → counted toward ALL markets (CBD, SW, NW, E, C)
- **Urban Core** → CBD + C
- **Far NW, FNW, Domain, Cedar Park** → NW
- **NC** → C
- **Bee Caves** → SW
- **Direct matches:** CBD, SW, NW, E, C counted toward themselves
- Multiple markets in one row (e.g., "CBD, SW, NW, E") → counted toward each listed market separately

Generated by: `create_demand_by_tenant_size_by_market.py` script
...
The script includes a robust `clean_sf_value()` function that handles various input formats:

| Input Format | Cleaning Applied |
|-------------|-----------------|
| `1,135 SF` | Remove "SF" suffix and comma |
| `5,333` | Remove comma |
| `2.87 acres` | Convert to SF (× 43,560) |
| `13,443 SF (expansion) & 29,621 SF (extension) = 43,064 SF` | Extract total after "=" |
| `38.669 ac or 1,684,410sf` | Prefer SF value over acres |
| `Suite 238 (894 SF) & Suite 234 (1,750 SF)` | Sum suite values |
| `for a total of 5,900 SF` | Extract total |
| `United States`, `N/A`, `-` | Set to null |
...
**Script Usage:**
```bash
python3 create_transaction_form_chart.py
...
### 10. create_demand_by_tenant_size_by_market.py (Market-Specific Demand by Tenant Size)

**Purpose:** Generate market-specific Office Demand by Tenant Size charts, broken out by submarket (CBD, SW, NW, E, C). Each row can be counted toward multiple submarkets based on inclusive market mapping rules.

**Data Sources:**
- **Tab 0:** "2025 +" - Current tenant requirements (743 rows)
- **Tab 1:** "DITM & Crab Trap MASTER Report (Through 2024)" - Historical data (6,971 rows)
- **Spreadsheet ID:** `1bzpRnUrpBH6l_zX7DtTypczZf5bpYVwqPUG3tzg2vec`

**Market Mapping Rules:**

The script applies inclusive market mapping where requirements can count toward multiple submarkets:

| Market String | Maps To | Example |
|--------------|---------|---------|
| Citywide, Flexible, Market Wide, Austin MSA, Austin Metro | **All markets** (CBD, SW, NW, E, C) | A "Citywide" requirement counts toward all 5 markets |
| Urban Core | CBD + C | Counts toward both CBD and Central |
| Far NW, FNW, Domain, Cedar Park | NW | Northwest consolidation |
| NC | C | Central |
| Bee Caves | SW | Southwest |
| CBD, SW, NW, E, C | Themselves | Direct mapping |
| CBD, SW, NW, E (multi-market) | Each listed market | Counted separately toward CBD, SW, NW, and E |

**Key Features:**
- **Inclusive counting:** Same row can appear in multiple market charts
- **No double-counting within a market:** Each row counted once per applicable market
- **Size categories:** Sub 10k SF, 10k-25k SF, 25k-50k SF, 50k-100k SF, Mega Requirements (100k+)
- **Color scheme:** Pennybacker (Sub 10k), Copper (10k-25k), Brass (25k-50k), Greenspace (50k-100k), Navy (Mega)
...
Generates 5 market-specific charts in `charts/office/`:

1. **requirements_demand_by_tenant_size_cbd.html** - CBD market
2. **requirements_demand_by_tenant_size_sw.html** - Southwest market
3. **requirements_demand_by_tenant_size_nw.html** - Northwest market
4. **requirements_demand_by_tenant_size_e.html** - East market
5. **requirements_demand_by_tenant_size_c.html** - Central market
...
**Script Usage:**
```bash
python3 create_demand_by_tenant_size_by_market.py
...
#   Reading Tab 0: '2025 +' data...
#     - Loaded 743 rows
#   Reading Tab 1: 'Through 2024' data...
#     - Loaded 6971 rows
#
# Step 3: Standardizing data...
#   Combined dataset: 5975 rows
#   Date range: 2018-01-02 00:00:00 to 2026-01-23 00:00:00
#
# Step 4: Setting up market mapping...
#   Testing market mapping:
#     'CBD, SW, NW, E' → ['CBD', 'SW', 'NW', 'E']
#     'Citywide' → ['CBD', 'SW', 'NW', 'E', 'C']
#     'Urban Core' → ['CBD', 'C']
#     'Far NW' → ['NW']
#     'NC' → ['C']
#     'Bee Caves' → ['SW']
...
#   Expanded to 14,523 market-specific records
#   Breakdown by submarket:
#     CBD: 2,845 records, 12,456,789 SF
#     SW: 3,124 records, 15,234,567 SF
#     NW: 3,567 records, 18,456,123 SF
#     E: 2,678 records, 11,234,456 SF
#     C: 2,309 records, 10,123,789 SF
#
# Step 6: Generating market-specific charts...
#
#   Generating chart for CBD...
#     ✓ Saved charts/office/requirements_demand_by_tenant_size_cbd.html
#
#   Generating chart for Southwest...
#     ✓ Saved charts/office/requirements_demand_by_tenant_size_sw.html
#
#   Generating chart for Northwest...
#     ✓ Saved charts/office/requirements_demand_by_tenant_size_nw.html
#
#   Generating chart for East...
#     ✓ Saved charts/office/requirements_demand_by_tenant_size_e.html
#
#   Generating chart for Central...
#     ✓ Saved charts/office/requirements_demand_by_tenant_size_c.html
#
# ================================================================================
# ✓ Complete!
# ================================================================================
...
**Use Case:** These market-specific charts enable:
- Analyzing demand trends by specific submarket
- Comparing tenant size mix across different markets
- Understanding which markets have more small vs. large tenant demand
- Identifying market-specific opportunities and trends
- Supporting submarket-specific leasing and investment decisions
...
- **Chart categories:** property-management (Aquila Operating KPIs), office, industrial, economic-indicators
